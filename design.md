...
belief they don't need to be exceptionally painful. Part of what sometimes makes conflict resolution difficult is when developers are asked to to merge the *effects* of offline changes. Reasoning about merging the effects of widely divergent histories is almost impossible. Instead, Replicache implements merge by *rewinding* the offline operations, and their effects, and then *replaying* them atop the latest server-side state. Your backend receives a series of delayed but otherwise normal RPCs. It's your responsibility to do something reasonable with these delayed RPCs. For many cases, just handling them normally yields the correct result. Sometimes something fancier is needed. Whatever the result, the Offline View guarantees that all clients snap into alignment on next sync.
