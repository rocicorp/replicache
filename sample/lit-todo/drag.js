...
async function handleDragStart(e, targetID, startIndex, getCurrentTodos, handleRender, handleDragEnd) {
...
async function handleDragMove(e) {
    ...
    const newIndex = Math.max(0, Math.min(todos.length, activeDrag.startIndex + dItem));
...
async function handleDragEnd() {
    ...
    const next = currentIndex == todos.length - 1 ? 1 : todos[currentIndex + 1].order;
